...
class PoController extends GeneralController
{
    public function processOrder() {
        try {
            ...
            \Stripe\Stripe::setApiKey(sc_config('po_secret'));
            $session = \Stripe\Checkout\Session::create([
                'customer_email' => auth()->user()->email,
                'payment_method_types' => ['po'],
                'line_items' => $lineItems,
                'success_url' => route("po.complete") . '?session_id={CHECKOUT_SESSION_ID}',
                'cancel_url' => route("po.cancel"),
            ]);
            $html = '<body>
                    ...
                    <script>
                        var stripe = Stripe("' . sc_config("po_public_key") . '");
                        stripe
                            .redirectToCheckout({
    ...
    public function complete(Request $request) {
        ...
        $transaction = ShopPoTransaction::where('session_id', $session_id)->first();
        if ($transaction) {
            return $this->invalidRequest();
        }
        ...
        try {
            ...
            ShopPoTransaction::create($dataInsert);

            //ShopOrder::updateInfo(["received" => $total], $orderID);
            (new ShopOrder)->updateStatus($orderID, $status = sc_config('po_order_status_success'), "Po Payment Success");
    ...
    public function cancel() {
        ...
        $msg = "You canceled po payment. Please try again";
        (new ShopOrder)->updateStatus($orderID, $status = sc_config('po_order_status_faild'), $msg);
        return redirect()->route('cart')->with(["error" => $msg]);
    }
}
