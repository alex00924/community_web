...
class CardController extends GeneralController
{
    ...
    //     $total = session('dataOrder')['total'];
    //     $currency = sc_currency_code();
        
    //     $arrCartDetails = session('arrCartDetail');
    //     $attributeGroups = ShopAttributeGroup::getList();
    //     foreach($arrCartDetails as $key => $cartDetail) {
    //         if ($cartDetail["attribute"]) {
    //             $attributes = json_decode($cartDetail["attribute"]);
    //             $attributeDetails = [];
    //             foreach($attributes as $attr_key => $option) {
    //                 $attributeDetails[$attributeGroups[$attr_key]] = $option;
    //             }
    //             $arrCartDetails[$key]["attribute"] = $attributeDetails;
    //         }
    //     }

    //     $description = json_encode($arrCartDetails); echo $description;
    //     try{
    ...
    //     var_dump($stripe);
    //     if ($stripe["amount"] == $total * 100 && $stripe["status"] == "succeeded" && $stripe["paid"] == true) {
    //         ShopOrder::updateInfo(["received" => $total], $orderID);
    //         (new ShopOrder)->updateStatus($orderID, $status = sc_config('card_order_status_success'), "Card Payment Success");
    //         return (new ShopCart)->completeOrder();
    //     } else {
    //         $msg = "Card Payment Faild. " . $stripe["failure_message"];
    //         (new ShopOrder)->updateStatus($orderID, $status = sc_config('card_order_status_faild'), $msg);
    //         return redirect()->route('cart')->with(["error" => $msg]);
    //     }
    // }
